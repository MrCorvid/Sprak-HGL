...
//            result.Add(new FunctionDefinition("bool", "toBool", new string[] { "var" }, new string[] { "value" }, new ExternalFunctionCreator.OnFunctionCall(API_toBool), functionDoc_toBool));
		...
		public SprakRunner(AST preParsedAst, FunctionDefinition[] functionDefinitions)
		{
			m_compileTimeErrorHandler = new ErrorHandler();
			m_runtimeErrorHandler = new ErrorHandler();
			m_ast = preParsedAst;
			
			var allFunctions = builtInFunctions.Concat(functionDefinitions).ToArray();
			
			var externalFunctionCreator = new ExternalFunctionCreator(allFunctions);
			
			var functionList = m_ast.getChild(1);
			foreach (AST funcAst in externalFunctionCreator.FunctionASTs)
			{
				functionList.addChild(funcAst);
			}

			Scope globalScope = CreateScopeTree(m_ast);
			if (m_compileTimeErrorHandler.getErrors().Count > 0) return;
